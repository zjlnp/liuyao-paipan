...
(function() {
  ...
  function performDivination(lines, data) {
    ...
    if (window.ZhuangGua && typeof calcGanZhi === 'function') {
      ...
      if (result.bian_gua) {
        // 变卦六亲随本卦宫五行（书中明确：变卦五行属性跟随本卦）
        result.bian_zhuanggua = window.ZhuangGua.installZhuangGua(
          { lines: bianLines, lines_display: result.bian_gua.lines_display, bagong: bianHexagram.bagong, upper: bianHexagram.upper, lower: bianHexagram.lower, parentPalace: benHexagram.bagong.palace },
          gz, data.hexagrams
        );
      }
    }
  ...
  window.zhouyiAPI = {
    ...
    divinate: function(lines, gzData) {
      return loadData().then(function(data) {
        if (!gzData && typeof calcGanZhi === 'function') { gzData = calcGanZhi(new Date()); }
        return performDivination(lines, data, gzData);
      });
    },
    clearCache: function() {
      if ('caches' in window) {
