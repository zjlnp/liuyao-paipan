  ...
  inputs.forEach((inp, idx) => {
    inp.addEventListener('keydown', e => {
      ...
      if (/^[6-9]$/.test(e.key)) {
        ...
        processingKey = true;
        setTimeout(() => { processingKey = false; }, 50);
...
async function doDivination() {
  ...
  divinating = true;
  try {
    if (!currentGzData) currentGzData = calcGanZhi(new Date());
    const result = await window.zhouyiAPI.divinate(lines, currentGzData);
...
function insertPaipanInfo(r) {
  ...
  // 公历日期用干支数据中的日期，不用系统时间
  var ds = gz.dateStr;
  var lu = gz.nongLi;
  var lunarDay = (typeof lu.d === 'number' && !isNaN(lu.d)) ? nongLiDayCN(lu.d) : lu.d;
  var nl = '农历'+(lu.isLeap?'闰':'')+lu.mn+'月'+lunarDay;
  var gzStr = gz.year.g+gz.year.z+' '+gz.month.g+gz.month.z+' '+gz.day.g+gz.day.z+' '+gz.hour.g+gz.hour.z;
  var xt = (Math.floor(gz.day.o/10)*10)%60;
  var xkMap = {0:'戌亥',10:'申酉',20:'午未',30:'辰巳',40:'寅卯',50:'子丑'};
  var xk = xkMap[xt] || '';
  var jq = gz.jieQi ? gz.jieQi.name : '';

  var h = '<div class="paipan-info">';
  ...
  for (var i=0; i<ss.length; i++) {
    if (i>0) h += ' ';
    h += '<b class="'+ss[i].cls+'">'+ss[i].label+' '+ss[i].value+'</b>';
  }
  h += '</div></div>';

  var card = document.getElementById('ben-card');
  if (!card) return;
  var old = card.querySelector('.paipan-info');
  if (old) old.remove();
  var header = card.querySelector('.gua-header');
  if (!header) return;
  var tmp = document.createElement('div');
  tmp.innerHTML = h;
  card.insertBefore(tmp.firstChild, header);
}
...
function render(r) {
  ...
  renderGuaCard('ben', r.ben_gua, r.changing_yaos, false, r.ben_zhuanggua);
  renderGuaCard('bian', r.bian_gua, r.changing_yaos, true, r.bian_zhuanggua);

  // 排盘信息（节气/干支/卦身/世身/神煞）插入本卦卡片顶部
  insertPaipanInfo(r);
...
function renderGuaCard(prefix, gua, changingYaos, isBian, zhuanggua) {
